...
1. 文件最后只剩下<4的时候，也就是需要读的char字数 >> 文件本身有的char数目，
比如：n=50 readbyte=23. 最后读到<4的时候，read下一次就不读了。
然后赋值的时候，只赋值最后这<4的char 给buf。

(至于n = 50, readbyte = 20 的情况，也就是最后一个也是4，whileloop readbyte<n, 
自动会跳出，不再继续读了。)

2. 文件char数目>>n,比如：文件有50个char，只要读23个char。read4会读满4个char，
但是最后我们只需要这3个char。所以最后赋值的时候，只需要最后3个。
